...
with mp_pose.Pose(
    ...
    while cap.isOpened():
        ...
        if 1 < try_count <= 6:
            if angle_count < 3:
                key_angle = random.choice(list(test_dict[try_key].keys()))
                val_result = test_dict[try_key].pop(key_angle)                  # 이거 위에 라인 포함 if 밖에서, 게속 재선언 되니까 중복
        ...
        try_count += 1  # try_count가 1<TRY_COUNT<6일 때 늘어야 한다
